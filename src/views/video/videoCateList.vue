...
<template>
    <div class="app-container calendar-list-container">
        ...
        <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit
            ...
            <el-table-column width="150px" align="center" label="视频时长" sortable>
                <template slot-scope="scope">
            ...
            <el-table-column width="100px" align="center" label="是否添加">
                <template slot-scope="scope">
                    <el-tag :type="scope.row.is_cate == 1 ? 'success' : 'danger'">
                        {{scope.row.is_cate == 1 ? '已添加' : '未添加'}}
            ...
            <el-table-column align="center" label="操作" width="230px" class-name="small-padding fixed-width">
                <template slot-scope="scope">
                    ...
                    <el-button size="mini" :type="scope.row.is_cate ? 'danger' : ''"
                               @click="handleModifyStatus(scope.row,!scope.row.is_cate)">
                        {{ scope.row.is_cate == 0 ? '添加' : '取消' }}
...
<script>
    import {fetchList, getCateVideo, addCateVideo, deleteCateVideo} from '@/api/video'
    ...
    export default {
        ...
        methods: {
            ...
            handleModifyStatus(row, status) {
                let data = {
                    cate_id: this.listQuery.cate_id,
                    video_id: row.id
                }
                if (row.is_cate) {
                    deleteCateVideo(data).then(res => {
                        this.getList();
                        this.$message({
                            message: '操作成功',
                            type: 'success'
