...
<template>
  <div class="app-container calendar-list-container">
    ...
    <el-button class="filter-item" style="margin-left: 10px;" @click="handleCreate" type="primary" icon="el-icon-edit">新增头条新闻</el-button>
    </div>
    ...
    :default-sort = "{prop: 'banner_no', order: 'ascending'}"
      ...
      <el-table-column width="auto" class="img-show" align="center" label="头条描述">
        <template slot-scope="scope">
          <span>{{ scope.row.desc }}</span>
        </template>
      </el-table-column>
      <el-table-column width="100px" align="center" label="跳转链接">
        <template slot-scope="scope">
        	<a v-if="scope.row.url" :href="scope.row.url" target="_blank" >{{scope.row.url}}</a>
          <span v-else>{{scope.row.url || '无'}}</span>
        </template>
      </el-table-column>
      <el-table-column width="100px" align="center" label="跳转参数">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="140px" style='width: 400px; margin-left:50px;'>
        ...
        <el-form-item label="头条详情" prop="desc">
       <el-input
		  type="textarea"
		  :rows="2"
		  placeholder="请输入头条内容"
        ...
        <el-form-item label="头条排序" prop="params">
